(function () {
  ...
  async function getSavedProjects() {
    return getStoredProjects(chrome.storage.sync, SYNC_PROJECTS_KEY, LEGACY_SYNC_KEY);
  }

  async function setSavedProjects(projects) {
    await chrome.storage.sync.set({ [SYNC_PROJECTS_KEY]: projects.map(normalizeProject) });
  }

  async function getSessionProjects() {
    return getStoredProjects(chrome.storage.local, SESSION_PROJECTS_KEY, LEGACY_SESSION_KEY);
  }

  async function setSessionProjects(projects) {
    await chrome.storage.local.set({ [SESSION_PROJECTS_KEY]: projects.map(normalizeProject) });
  }
