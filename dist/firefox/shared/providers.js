(function () {
  ...
  async function getProviderConfig() {
    const legacy = await ext.storage.local.get({ anthropic_api_key: "" });
    const result = await ext.storage.local.get({ [PROVIDER_CONFIG_KEY]: null });
  ...
  async function saveProviderConfig(config) {
    ...
    await ext.storage.local.set({ [PROVIDER_CONFIG_KEY]: next });
    if (next.provider === "anthropic") {
      await ext.storage.local.set({ anthropic_api_key: next.apiKey });
    }
    return next;
  }
